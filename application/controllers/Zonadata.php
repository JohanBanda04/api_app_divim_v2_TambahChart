...
class Zonadata extends CI_Controller
{
    ...
    public function dataITKByRangeTanggal($tanggalAwal, $tanggalAkhir) //parameter didapet dari url
    {
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'GET' || $this->uri->segment(3) == '') {
            json_output(400, array('status' => 400, 'message' => 'Bad request.'));
            ...
            if ($check_auth_client == true) {
                $response = $this->MyModel->auth();
                if ($response['status'] == 200) {
                    $resp = $this->MyModel->dataITKByRangeTanggal($tanggalAwal, $tanggalAkhir);
                    json_output($response['status'], $resp);
                }
            }
        }
