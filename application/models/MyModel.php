...
class MyModel extends CI_Model {
    ...
    { 
        $q  = $this->db->select('password,id,role,nama,username,whatsapp,id_zona')->from('users')->where('username',$username)->get()->row();
        if($q == ""){
            // echo 'wkwk';
            ...
            if (hash_equals($hashed_password, crypt($password, $hashed_password)) || $password === $hashed_password) {
               ...
               if ($this->db->trans_status() === FALSE){
                  ...
                  return array('status' => 200,'message' => 'Successfully login.',
                                ...
                                'whatsapp' => $whatsapp,'id_zona'=>$id_zona
                                );
               }
            } else {
    ...
    public function zonaByID($table, $id)
    {
        return $this->db->select('*')->from($table)->where('id',$id)->order_by('id','desc')->get()->row();
    }
    ...
    public function izintinggalByZona($zona_id,$table_name){
        return $this->db->query("SELECT * FROM $table_name WHERE id_zona = $zona_id")->result();
    }

    public function izintinggalByZonaAndStatus($zona_id,$table_name,$status){
        return $this->db->query("SELECT * FROM $table_name WHERE id_zona = '$zona_id' and status='$status' ")->result();
    }

    public function izintinggalByZonaAndNotStatus($zona_id,$table_name,$status){
        return $this->db->query("SELECT * FROM $table_name WHERE id_zona = '$zona_id' and status !='$status' ")->result();
    }

    public function izintinggalAllForCount($table_name){
        return $this->db->query("SELECT count(id) as jumlah_data FROM $table_name ")->result();
    }

    public function getDataByNoPaspor($no_paspor,$table_name){
        return $this->db->query("SELECT * FROM $table_name WHERE no_paspor = '$no_paspor'")->result();
    }

    public function izintinggalByStatusandZonaID($zona_id,$status,$table_name){
        return $this->db->query("SELECT * FROM $table_name WHERE id_zona = $zona_id and status ='$status' ")->result();
    }

    public function izintinggalAllByStatus($status,$table_name){
        return $this->db->query("SELECT * FROM $table_name WHERE status ='$status' ")->result();
    }

    public function bahanBeritaByTanggal($tanggal){
        return $this->db->query("SELECT * FROM `bahan_berita` WHERE DATE(berita_when) = '$tanggal' ORDER BY berita_when
        ")->result();
    }
    ...
    public function dataITKByRangeTanggal($tanggalAwal, $tanggalAkhir){
        return $this->db->query("SELECT * FROM `data_itk` WHERE DATE(created_at) BETWEEN '$tanggalAwal' AND '$tanggalAkhir'")->result();
    }
